...
function readHiddenToken(prompt: string): Promise<string> {
  if (!process.stdin.isTTY || !process.stdout.isTTY || typeof process.stdin.setRawMode !== 'function') {
    throw new DodoError('NOT_SUPPORTED', 'an interactive terminal is required for OS credential entry', {
      recovery: 'use --token-env or --token-file in headless environments',
    });
  }
