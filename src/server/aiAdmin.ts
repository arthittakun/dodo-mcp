...
const ACTIONS = ['status','trust.set','schedule.list','schedule.propose','schedule.show','schedule.history','schedule.approve','schedule.revoke','memory.pending','memory.show','memory.list','memory.approve','memory.reject','memory.reverify','memory.prune','memory.learning.pending','memory.learning.show','memory.learning.review','agent.skill.pending','agent.skill.show','agent.skill.review','desktop.status','desktop.policy','approvals.pending','approvals.approve','approvals.deny','auth.pending','auth.approve','auth.deny','auth.addClient','auth.listClients','auth.grants','auth.revoke','audit.recent','recover.list','recover.resolve','recover.rollback','deployment.targets','deployment.configure','deployment.prepare','deployment.list','deployment.inspect','deployment.compare','deployment.build','deployment.apply','deployment.observe','deployment.source_preview','deployment.rollback_prepare','deployment.maintenance','recovery.status','recovery.evidence.list','recovery.evidence.inspect','recovery.mark','recovery.pin','recovery.cleanup.preview','recovery.drift.scan','recovery.drift.acknowledge','recovery.configure','recovery.checkpoint','recovery.restore_status','recovery.checkpoint_list','recovery.checkpoint_inspect','recovery.checkpoint_create','recovery.recovery_session_list','recovery.recovery_session_inspect','recovery.recovery_session_begin','recovery.recovery_session_end','recovery.restore_preview','recovery.restore_apply'] as const;
/**
