...
import { partialTestProgress } from './gate-progress.mjs';
...
try {
  ...
  for (const major of [22, 24]) {
    const directory = path.resolve(root, '../.dodo-ci-evidence', run, `windows-node-${major}`);
    ...
    console.log(JSON.stringify({ nodeMajor: major, summary, partial, failures }, null, 2));
