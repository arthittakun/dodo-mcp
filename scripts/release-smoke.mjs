...
function parseArgs() {
  ...
  if (!out.tarball || !out.output) throw new Error('usage: release-smoke.mjs --tarball FILE --output FILE');
  return out;
}
...
const install = fs.mkdtempSync(path.join(os.tmpdir(), 'dodo-release-install-'));
const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'dodo-release-workspace-'));
const config = fs.mkdtempSync(path.join(os.tmpdir(), 'dodo-release-config-'));
let running;
let stdio;
let httpClient;
try {
  ...
  console.log(`[release-smoke] PASS | cli=${report.cliVersion} | full=${fullCount} | compact=${compactCount}`);
} finally {
  ...
  for (const directory of [install, workspace, config]) fs.rmSync(directory, { recursive: true, force: true, maxRetries: 3 });
}
