...
describe('workspace access migration', () => {
  it.each([2,3])('upgrades schema %s preserving scopes, revocations and workspace separation', version => {
    ...
    try {
      ...
      db.exec('DROP TABLE resource_refs; DROP TABLE resource_objects; DROP TABLE project_registry; DROP TABLE chat_permissions; DROP TABLE usage_consents; DROP TABLE schedules; DROP TABLE schedule_runs');
