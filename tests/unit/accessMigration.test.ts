...
describe('workspace access migration', () => {
  it.each([2,3])('upgrades schema %s preserving scopes, revocations and workspace separation', version => {
    ...
    try {
      ...
      db.exec('DROP TABLE brain_edges; DROP TABLE brain_nodes; DROP TABLE brain_file_cache; DROP TABLE brain_runs; DROP TABLE brain_index_state; DROP TABLE resource_refs; DROP TABLE resource_objects; DROP TABLE project_registry; DROP TABLE chat_permissions; DROP TABLE usage_consents; DROP TABLE schedules; DROP TABLE schedule_runs');
