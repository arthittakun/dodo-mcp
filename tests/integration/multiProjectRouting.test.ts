...
describe('concurrent project routing on modern HTTP + OAuth',()=>{
  it('keeps clients A/B isolated, parallel jobs, stale epochs and project selection without default ACL',async()=>{
    ...
    try{
      ...
      const jobs=await Promise.all([call(clients[0]!,'dodo_exec',{...contextA,operation:'exec_command',args:{program:'node',args:['-e','process.stdin.resume();process.stdin.once("end",()=>console.log("A test passed"))'],idempotencyKey:'parallel-project-a'}}),call(clients[1]!,'dodo_exec',{...contextB,operation:'exec_command',args:{program:'node',args:['-e','process.stdin.resume();process.stdin.once("end",()=>console.log("B test passed"))'],idempotencyKey:'parallel-project-b'}})]);
      ...
      // Release both real jobs only after observing them alive together. A fixed
      // one-second delay measured ACL/startup speed, not project concurrency.
      await Promise.all(jobs.map((r,i)=>call(clients[i]!,'dodo_exec',{...(i===0?contextA:contextB),operation:'job_input',args:{jobId:(r.data as {jobId:string}).jobId,data:'',closeStdin:true}})));
