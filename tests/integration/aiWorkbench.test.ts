...
describe.skipIf(!fs.existsSync(chromium.executablePath()))('owner AI workbench in Chromium',()=>{
  it('adds provider/profile/project in real UI, runs an agent on B, reconnects, and renders narrow/dark/light safely',async()=>{
    ...
    try{
      ...
      await page.getByRole('button',{name:'บันทึก connection',exact:true}).click();await page.getByRole('heading',{name:'Protocol Fixture',exact:true}).waitFor();
      await page.getByRole('button',{name:'โหลดรายชื่อโมเดล',exact:true}).click();await page.getByRole('code').filter({hasText:'fixture-model'}).waitFor();expect(await page.getByText(/พบ 1 โมเดล/).isVisible()).toBe(true);
      ...
      // A browser click completes before its async request; wait for the server-confirmed save.
      await page.getByText('เพิ่มในทะเบียนแล้ว ตั้งค่าสิทธิ์ก่อนใช้งาน',{exact:true}).waitFor();
      ...
      await page.getByRole('button',{name:'Providers & Profiles',exact:true}).click();await page.locator('#wb-body input[name=model]').fill('fixture-model');await page.getByText('ตั้งค่าขั้นสูง (สิทธิ์, tool calling, token และราคา)',{exact:true}).click();await page.locator('#wb-body input[name=write]').check();await page.locator('#wb-body input[name=tools]').check();await page.locator('#wb-body input[name=images]').check();await page.getByRole('button',{name:'สร้าง Agent',exact:true}).click();await page.getByRole('heading',{name:'Coding',exact:true}).waitFor();
