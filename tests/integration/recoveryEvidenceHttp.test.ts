...
describe('R04 private dashboard and real OAuth evidence',()=>{
 ...
 it.skipIf(!fs.existsSync(chromium.executablePath()))('browser adds project ON, verifies, pins/names, shows stale evidence and quota errors at desktop/narrow sizes',async()=>{
  ...
  try{
    ...
    const checkpointCount=()=>c.server.services.store.db.prepare('SELECT COUNT(*) n FROM recovery_snapshots').get();const beforePreview=checkpointCount();await page.getByRole('button',{name:'ตรวจแผนล้างตาม retention',exact:true}).click();await page.getByRole('heading',{name:'ตรวจรายการก่อนลบ',exact:true}).waitFor();expect(checkpointCount()).toEqual(beforePreview);
    // Real quota reservation failure, not a canned browser response.
    fs.writeFileSync(path.join(c.fixtureDir,'large-source.txt'),'x'.repeat(2*1024*1024));await page.getByText('โควตา ระยะเก็บ และโฟลเดอร์ที่ไม่สำรอง',{exact:true}).click();await page.locator('input[name=maintenanceQuota]').fill('0.001');await page.getByRole('button',{name:'ตรวจแผนนโยบายสำรอง',exact:true}).click();await page.getByRole('button',{name:'ยืนยันบันทึกนโยบาย',exact:true}).click();await page.locator('.swal2-confirm').click();await expect.poll(()=>c.server.services.recovery!.policy().projectBytes).toBe(Math.round(0.001*1073741824));await page.getByRole('button',{name:'สร้างจุดกู้คืนตอนนี้',exact:true}).click();
